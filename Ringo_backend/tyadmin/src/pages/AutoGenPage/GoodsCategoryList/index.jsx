...
import { addGoodsCategory, queryGoodsCategory, removeGoodsCategory, updateGoodsCategory, queryGoodsCategoryVerboseName, queryGoodsCategoryListDisplay, queryGoodsCategoryDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateGoodsCategory(value, current_id);
      hide();
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
                      ...
                      message: '类目级别为必填项',
                             ...
                             valueEnum:{"1":"一级类目","2":"二级类目","3":"三级类目"}
                        },
                      {
                             title: '父类目级别',
                        ...
                        renderFormItem: (item, {value, onChange}) => {
                                          return dealForeignKeyField(item, value, onChange, parent_categoryForeignKeyList);
                                  },
                        render: (text, record) => {
                              return renderForeignKey(text, parent_categoryVerboseNameMap);
                            },
  ...
  useEffect(() => {
    queryGoodsCategoryDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryGoodsCategoryListDisplay().then(value => {
      setColumnsStateMap(value)
                                ...
                                const [parent_categoryForeignKeyList, setParent_categoryForeignKeyList] = useState([]);
                                useEffect(() => {
                                queryGoodsCategory({all: 1}).then(value => {
                                     setParent_categoryForeignKeyList(value);
                                });
                                }, []);
                                const [parent_categoryVerboseNameMap, setParent_categoryVerboseNameMap] = useState([]);
                                useEffect(() => {
                                queryGoodsCategoryVerboseName().then(value => {
                                    setParent_categoryVerboseNameMap(value);
                                });
                                }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryGoodsCategory, table_columns, '物品类别-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索物品类别" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '物品类别-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryGoodsCategory({ ...params, sorter, filter })}
