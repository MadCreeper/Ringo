...
def get_string_tag(offer_list):
    string_list = [i.goods_brief + i.goods_desc for i in offer_list]
    return ''.join(string_list)

# 需要验证string_tag != ''
def sort_by_similarity(needs_list:list, string_tag):
    needs_list.sort(key=lambda i: synonyms.compare(string_tag, i.goods_brief + i.goods_desc) if i else 0, reverse=True)
    return needs_list
